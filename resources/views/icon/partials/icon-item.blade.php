<li class="flex items-center py-4 px-6 hover:bg-gray-50 transition duration-150 ease-in-out">
    <img src="{{ Storage::url($icon->image_path) }}" alt="アイコン1" class="w-20 h-20 object-cover rounded-lg">
    ...
    <form action="{{ route('icon.destroy',['icon'=>$icon->id]) }}" method="POST" class="ml-6" onsubmit="return window.confirm('選択したアイコンを本当に削除しますか？')">
        @csrf
        @method('DELETE')
